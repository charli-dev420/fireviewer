...
export function FireWarningIncidentsPage() {
  ...
  return <>
    ...
    <div className={`fw-page fw-standard-page fw-incidents-page ${lowData ? 'is-low-data' : ''}`}>
      ...
      {items.length ? <ul className="fw-incident-cards">{items.map((item) => <li key={item.fire_id}><a className="fw-incident-card-link" href={`/incendie/${item.fire_id}`} aria-label={`Ouvrir la fiche ${item.canonical_name}`}><article><div className="fw-incident-card-status"><span>{item.status}</span><small>{item.verification === 'verified' ? 'Information validée' : 'Information corroborée'}</small></div><div><small>{item.fire_id}</small><h2>{item.canonical_name}</h2><p>{tab === 'archive' ? `Dernière activité : ${humanDate(item.last_observed_at)}` : `Dernière évolution publiée · ${age(item.last_observed_at)}`}</p></div><div className="fw-incident-card-actions"><span><PublicIcon name="clock" size={16} />{humanDate(item.last_observed_at)}</span><span className="fw-incident-card-cta">{tab === 'archive' ? 'Consulter l’archive' : 'Voir la page de l’incendie'}<PublicIcon name="arrow" size={16} /></span></div></article></a></li>)}</ul> : null}
    </div>
  </>;
}
