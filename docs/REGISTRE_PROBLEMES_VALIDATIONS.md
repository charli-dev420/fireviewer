...
| G1-007 | VÉRIFIÉ le 14 juillet 2026 | Un checkout Windows du tag v4 convertissait les JSON hashés en CRLF avec `core.autocrlf=true`; Alembic ignorait `FV_DATABASE_URL` dans le runbook. | `.gitattributes` force LF pour catalogue, provenance et attribution. Alembic applique désormais `FV_DATABASE_URL` avant l'URL de repli ; le tag source `spatial-die-pontaix-r1-v4-fix1` conserve le tag associé à la release v4 inchangé. | Clone Windows du tag `spatial-die-pontaix-r1-v4-fix1` (`9d9f0704d1b5d2936dba95e739a48c79ab850955`, contenant le correctif fonctionnel `a42e55c`), hashes LF conformes, 68 tests UI, 11 tests spatiaux, 20 E2E, 88/88 backend et restauration SQLite passés. |
| G1-008 | VÉRIFIÉ le 14 juillet 2026 | Le correctif G1 pouvait rester seulement sur une branche ou un tag, sans être intégré au parcours courant. | La PR #10 est fusionnée dans `main` au commit `6ae41d3fc6e5c9be9c9b3050902c593c6f0a196f`. Les tags v4 et v4-fix1 restent distincts, annotés et ancêtres de `main`. | API GitHub, `git ls-remote` et référence distante `origin/main` concordent. Aucun binaire n'a été ajouté dans Git. |

Passe corrective PR #13 — socle de l'administration des zones.

| ID | État de la passe | Problème | Correction conservée | Preuve exécutée ou limite |
| --- | --- | --- | --- | --- |
| ADM-001 | VÉRIFIÉ le 14 juillet 2026 | Le navigateur déduisait localement un rôle depuis les claims d'un JWT, ce qui permettait d'ouvrir le shell avec un token forgé. | Le bearer est opaque côté UI. `GET /api/v1/admin/session` vérifie réellement signature JWT/OIDC et rôle serveur avant tout montage privé ; `401` et `403` purgent la session. | Test RS256, refus de `alg:none`, refus du rôle non-admin, tests jsdom et E2E bureau/mobile exécutés. |
| ADM-002 | VÉRIFIÉ le 14 juillet 2026 | Les routes client et API ne suivaient pas le même format d'identité de zone et les réponses privées pouvaient être mises en cache. | Le contrat est `DIE-PONTAIX-08` en majuscules avec révision numérique ; OpenAPI est régénéré. Le middleware impose `Cache-Control: no-store` sur `/api/v1/admin/*`. | Tests API, TypeScript, OpenAPI contrôlé par le test de contrat et E2E exécutés. |
| ADM-003 | VÉRIFIÉ le 14 juillet 2026 | Des écritures SQL directes pouvaient créer un paquet ou une publication incohérente, modifier une vérification, relier une autre révision ou supprimer l'historique. | Migration additive `b4d1e6f9a730` : état initial brouillon, rapport de vérification concluante, lien de révision immuable, transitions autorisées, événements append-only et refus de suppression. | 108 tests backend, 90,11 % de couverture, migration upgrade/idempotence/check/downgrade et restauration SQLite avec empreintes de triggers exécutés. |
| ADM-004 | limite active | Les pages admin pourraient être confondues avec une gestion complète des zones. | Les écrans indiquent désormais leur statut préparatoire et les mutations restent `501` tant que le service CRUD transactionnel ne les traverse pas réellement. | NON VÉRIFIÉ : création, dépôt, prévisualisation privée rendue et publication depuis l'interface. |
| ADM-005 | limite active | Les guards de persistance ajoutés sont propres à SQLite. | Aucun faux équivalent PostgreSQL n'a été déclaré. | NON VÉRIFIÉ : PostgreSQL/PostGIS, connexion OIDC interactive et isolation de stockage privée en déploiement. |
