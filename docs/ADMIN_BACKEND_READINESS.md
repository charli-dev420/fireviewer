...
- **G1 hébergé : code de raccordement présent, infrastructure réelle NON VÉRIFIÉE.** Le backend
  possède un point d'entrée Vercel, accepte Neon/PostgreSQL et Vercel Private Blob, mais aucun
  projet Neon, store Blob ou déploiement Vercel n'a été exercé dans cet audit.
- **G2 : non atteint.** Les identités nominatives, la MFA, les territoires, les secondes
  approbations, le circuit complet contributions/médias/consentements, la supervision SLM et le
  téléversement direct des gros packages restent à implémenter.
...
| Stockage privé | Partiel | Abstraction locale/Vercel Blob, objets immuables, lecture privée | Téléversement navigateur direct/multipart pour les gros packages |
...
| OIDC/JWT serveur | Partiel | Validation JWKS, issuer, audience et rôles | Parcours frontend OIDC, comptes nominatifs et MFA |
| RBAC métier | Partiel | Rôles vérifiés sur les routes sensibles | Utilisateurs, territoires, matrice persistée et séparation complète des pouvoirs |
...
| Packages 3D locaux | Partiel | Révisions, archives, validation, preview et publication | Téléversement hébergé direct et tests de gros fichiers |
| Publication atomique | Partiel | Machine d'état package/publication et rollback | Seconde approbation, réauthentification et diff complet |
...
|---|---|
| Connexion locale | Fonctionnelle G1 |
| MFA, profil et sécurité nominative | Manquants |
| Tableau de bord | Fonctionnel en lecture, actions limitées |
...
| Modèles et couches 3D | Registre et import local disponibles ; production hébergée incomplète |
...
1. Implémenter l'identité nominative OIDC côté frontend et la MFA.
2. Persister utilisateurs, rôles, territoires et délégations.
3. Implémenter les secondes approbations et la réauthentification des actions critiques.
4. Construire contributions, médias, consentements et retrait de consentement de bout en bout.
5. Remplacer l'upload HTTP traversant FastAPI par un upload client multipart direct vers Blob,
   suivi d'une finalisation serveur vérifiée.
6. Implémenter le runner de jobs et le reconciler RunPod avec budgets et arrêt d'urgence.
7. Ajouter sauvegarde/restauration PostgreSQL, tests de charge Neon et exercice de panne.
8. Exécuter les scénarios E2E et la recette visuelle avec le navigateur autorisé par le projet.
...
- **Gate G2 :** tous les blocages ci-dessus sont fermés et les identités sont nominatives.
